...
class SlackAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        rule_get = rule.get
        self.slack_webhook_url = rule_get('slack_webhook_url', None)
        if isinstance(self.slack_webhook_url, str):
            self.slack_webhook_url = [self.slack_webhook_url]
        self.slack_proxy = rule_get('slack_proxy', None)
        self.slack_username_override = rule_get('slack_username_override', 'elastalert')
        self.slack_channel_override = rule_get('slack_channel_override', '')
        if isinstance(self.slack_channel_override, str):
            self.slack_channel_override = [self.slack_channel_override]
        self.slack_title_link = rule_get('slack_title_link', '')
        self.slack_title = rule_get('slack_title', '')
        self.slack_emoji_override = rule_get('slack_emoji_override', ':ghost:')
        self.slack_icon_url_override = rule_get('slack_icon_url_override', '')
        self.slack_msg_color = rule_get('slack_msg_color', 'danger')
        self.slack_parse_override = rule_get('slack_parse_override', 'none')
        self.slack_text_string = rule_get('slack_text_string', '')
        self.slack_alert_fields = rule_get('slack_alert_fields', '')
        self.slack_ignore_ssl_errors = rule_get('slack_ignore_ssl_errors', False)
        self.slack_timeout = rule_get('slack_timeout', 10)
        self.slack_ca_certs = rule_get('slack_ca_certs')
        self.slack_attach_kibana_discover_url = rule_get('slack_attach_kibana_discover_url', False)
        self.slack_kibana_discover_color = rule_get('slack_kibana_discover_color', '#ec4b98')
        self.slack_kibana_discover_title = rule_get('slack_kibana_discover_title', 'Discover in Kibana')
        self.slack_attach_opensearch_discover_url = rule_get('slack_attach_opensearch_discover_url', False)
        self.slack_opensearch_discover_color = rule_get('slack_opensearch_discover_color', '#ec4b98')
        self.slack_opensearch_discover_title = rule_get('slack_opensearch_discover_title', 'Discover in Opensearch')
        self.slack_footer = rule_get('slack_footer', '')
        self.slack_footer_icon = rule_get('slack_footer_icon', '')
        self.slack_image_url = rule_get('slack_image_url', '')
        self.slack_thumb_url = rule_get('slack_thumb_url', '')
        self.slack_author_name = rule_get('slack_author_name', '')
        self.slack_author_link = rule_get('slack_author_link', '')
        self.slack_author_icon = rule_get('slack_author_icon', '')
        self.slack_msg_pretext = rule_get('slack_msg_pretext', '')
        self.slack_attach_jira_ticket_url = rule_get('slack_attach_jira_ticket_url', False)
        self.slack_jira_ticket_color = rule_get('slack_jira_ticket_color', '#ec4b98')
        self.slack_jira_ticket_title = rule_get('slack_jira_ticket_title', 'Jira Ticket')
