...
class RuleType(object):
    ...
    def add_match(self, event):
        ...
        # Use local copy of deepcopy to reduce attribute lookup overhead
        deepcopy = copy.deepcopy
        self.matches.append(deepcopy(event))
...
class PercentageMatchRule(BaseAggregationRule):
    ...
    def check_matches(self, timestamp, query_key, aggregation_data):
        ...
        percentage_violation = self.percentage_violation
        if percentage_violation(match_percentage):
            # Only lookup ts_field and timestamp_field once for efficiency
            ts_field = self.rules['timestamp_field']
            match = {ts_field: timestamp, 'percentage': match_percentage, 'denominator': total_count}
            percentage_format_string = self.rules.get('percentage_format_string', None)
    ...
    def percentage_violation(self, match_percentage):
        rules = self.rules
        # Do not cache lookups of possible-mutable self.rules but optimize by local bind for hot path
        if 'max_percentage' in rules and match_percentage > rules['max_percentage']:
            return True
        if 'min_percentage' in rules and match_percentage < rules['min_percentage']:
            return True
        return False
