...
class PercentageMatchRule(BaseAggregationRule):
    ...
    def get_match_str(self, match):
        rules = self.rules  # local alias to avoid attribute access on every use
        percentage_format_string = rules.get('percentage_format_string', None)
        min_percentage = rules.get('min_percentage')
        max_percentage = rules.get('max_percentage')
        denominator = match['denominator']
        percentage = match['percentage']

        if percentage_format_string and percentage_format_string.startswith('{'):
            formatted_percentage = percentage_format_string.format(percentage)
        elif percentage_format_string:
            formatted_percentage = percentage_format_string % (percentage)
        else:
