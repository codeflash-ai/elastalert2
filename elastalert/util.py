...
def lookup_es_key(lookup_dict, term):
    ...
    """
    # Fast path: avoid _find_es_dict_by_key if term is a direct key
    if term in lookup_dict:
        return lookup_dict[term]
    _fesdbk = _find_es_dict_by_key  # local alias for speed
    value_dict, value_key = _fesdbk(lookup_dict, term)
