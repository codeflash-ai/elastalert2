...
def ts_to_dt(timestamp):
    if isinstance(timestamp, datetime.datetime):
        return timestamp
    # This branch is very hot and is responsible for most CPU, so use the cache
    # Accepts only string
    return _parse_ts_cached(timestamp)
...
# Use a global cache for timestamp parsing to avoid expensive repeated parsing
# Only cache for string, not datetime input to preserve semantics
@functools.lru_cache(maxsize=16384)
def _parse_ts_cached(timestamp: str):
