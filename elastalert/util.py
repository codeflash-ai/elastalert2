...
def _find_es_dict_by_key(lookup_dict: dict, term: str, string_multi_field_name: str = "keyword") -> tuple[dict, str]:
    ...
    # Pre-build joined substrings to avoid repeated joins in loop
    if subkeys_len > 2:
        join_cache = [''] * (subkeys_len - 1)
        curr = subkeys[0]
        for i in range(1, subkeys_len - 1):
            curr = curr + "." + subkeys[i]
            join_cache[i - 1] = curr

    for i in range(subkeys_len, 0, -1):
        # Use pre-computed joins when available
        if i == 1:
            root = subkeys[0]
        elif i == subkeys_len:
            root = term
