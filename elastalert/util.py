...
def lookup_es_key(lookup_dict, term):
    ...
    """
    value_dict, value_key = _lookup_es_key_fastpath(lookup_dict, term)
...
def get_version_from_cluster_info(client):
    ...
    return esversion
def _lookup_es_key_fastpath(lookup_dict, term):
    """Fast direct key or shallow nested lookup; returns (dict, key) or (None, None)."""
    if not isinstance(lookup_dict, dict) or not term or not isinstance(term, str):
        return None, None
    if '.' not in term and '[' not in term:
        # Simple flat key, avoid calling _find_es_dict_by_key
